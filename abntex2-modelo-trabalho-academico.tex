...
% ---
\titulo{Modelos de qualidade de produto de software com perspectivas sobre métodos ágeis, garantia de qualidade e testes}
...
Já as técnicas utilizadas no processo de QA podem ser categorizadas em dois grupos \cite{Huo:2004:SQA:1025117.1025549,Naik2011}\footnote{Para maiores detalhes sobre cada uma das técnicas de QA confira o anexo \emph{*a ser inserido*}}: % FIXME: inserir anexo
...
% FIXME: ver esse modelo
% modelo de qualidade que foca no controle da dívida técnica
...
\citeonline{fowler2011} relata outra situação bastante comum que é a crença em algo chamado \emph{hipótese da qualidade negociável}, que diz que é possível realizar uma troca entre recursos: ``é melhor para o produto adicionar um recurso visível para o usuário (qualidade externa) do que refatorar código (qualidade interna)''.
...
\citeonline{crispin2009} reforça que a diferença crítica entre um projeto ágil e um tradicional é o rápido \emph{feedback} provido pelos testes, pois permite que o projeto avance rapidamente, sem nenhuma surpresa desagradável no final de uma longa etapa de desenvolvimento. Ser ``ágil'' significa ter qualidade com eficiência.
...
Sendo assim, \citeonline{crispin2009} apresentam uma lista de princípios derivados dos princípios do Manifesto Ágil para guiar testadores adentrando os métodos ágeis, lembrando que o foco deve ser na atitude e mentalidade da pessoa, e não estritamente nas habilidades técnicas:
...
\begin{figure}[H]
    ...
    \legend{Fonte: \citeonline{crispin2009} [Adaptação do autor]}
\end{figure}

Uma questão importante é o dimensionamento do grupo de testes. Qual a proporção correta? Para \citeonline{crispin2009} não existe valor fixo, ele irá variar com as necessidades da equipe, as habilidades dos membros, complexidade do projeto, etc. Elas já trabalharam em equipes onde a proporção testador-desenvolvedor foi de 1:20 até 1:1.
...
\subsubsection{O Alicerce dos Pilares}
\citeonline{galen2015} apresenta alguns conceitos que considera como a base de toda equipe, que permeiam o modelo e devem ser continuamente utilizados durante um projeto.
% \begin{itemize}
%     \item Posse de toda a equipe
%     \item Saber construir a coisa certa do jeito certo
%     \item Métricas ágeis saudáveis
%     \item Governança % FIXME: steering via center of excellence of community of practice
%     \item Equilíbrio entre os 3 pilares: avaliação, recalibragem e melhoria contínua
% \end{itemize}

\begin{description}
    \item[Posse de toda a equipe] \hfill \\
        Apesar dos métodos ágeis pregarem a derrubada da divisão entre desenvolvedores e testadores, essa barreira ainda se mantém, então é essencial que haja ênfase constante no engajamento de todos os membros do time. Caso exista um problema na qualidade do produto, a culpa não é dos testadores, mas de todo o time.
    \item[Saber construir a coisa certa do jeito certo] \hfill \\
        Se engajar com os clientes para garantir que as histórias de usuário realmente reflitam as funcionalidades desejadas, ter tarefas que sigam a definição de conclusão (DoD) acordada pelo time e de maneira consistente são algumas das maneiras de tentar criar um produto de qualidade que irá agradar ao cliente.
    \item[Métricas ágeis saudáveis] \hfill \\
        \citeonline{galen2015} defente a migração de métricas funcionais para métricas baseadas em equipe, pois as métricas ágeis são fundamentalmente diferentes, e cada equipe deve utilizar métricas diferentes.
    \item[Governança] \hfill \\
        A aplicação dos princípios ágeis não deve se ater apenas à equipe de desenvolvimento e testes, mas também à liderança. A posse, responsabilidade, e direção do projeto é compartilhada por todos.
    \item[Equilíbrio entre os 3 pilares: avaliação, recalibragem e melhoria contínua] \hfill \\
        Para gerenciar e efetivamente produzir um produto de qualidade é necessário realizar a conexão e alcançar um equilíbrio entre os três pilares.
\end{description}
...
\begin{itemize}
    ...
    \item Ataque da infraestrutura técnica no backlog % FIXME: não entendi esse ponto
    \item \emph{Feedback} visual
    \item Prática ativa de Desenvolvimento Orientado a Testes de Aceitação (\emph{Acceptance Test-Driven Development} - ATDD) e Desenvolvimento Guiado por Comportamento (\emph{Behaviour-Driven Development} - BDD) % FIXME: significado das siglas
\end{itemize}

A estratégia baseada em pirâmides (apresentadas nas figuras \ref{fig:testing-pyramid-traditional} e \ref{fig:testing-pyramid-agile}) são baseadas no conceito criado por \citeonline{cohn2009}, onde a ideia principal é indicar que devem existir mais testes de baixo nível (unitários) do que testes de alto nível (interface). \citeonline{fowler2012} acredita que se uma falha for detectada em um teste de alto nível então além de um \emph{bug} no código também foi detectada uma deficiência nos testes unitários.

A situação apresentada na \autoref{fig:testing-pyramid-traditional} é considerada por \citeonline{scott2012} um anti-padrão (conceito abordado por \citeonline{nidia2015}), chamado jocosamente por ele de ``cone de sorvete de testes''. \citeonline{galen2015} considera este problema comum nos métodos tradicionais de desenvolvimento, onde os testadores ficam dentro de sua área de conforto escrevendo testes de interfaces e os desenvolvedores veem pouco valor em investir tempo na criação dos testes unitários, principalmente se estiverem sobre pressão para entregar código e funcionalidades dentro do prazo.
...
% \begin{figure}[H]
%     \centering
%     \caption{Pirâmides de testes automatizados: ágil e tradicional}
%     \graphicspath{ {./graphics/agile/} }
%     \includegraphics[scale=0.65]{testing-pyramid-combined}
%     \label{fig:testing-pyramid-combined}
%     \legend{Fonte: \citeonline{galen2015}}
% \end{figure}
...
\citeonline{galen2015} faz um aviso importante: não deve-se automatizar 100\% dos testes. Isso é um objetivo fútil, e o que determinará o número ideal de testes é a análise das histórias do usuário para que exista uma cobertura adequada em todos os níveis, não podendo de levar em consideração o valor e complexidade na criação de cada um.
...
\begin{itemize}
    ...
    \item Planejamento de testes @ \emph{release} e níveis de sprint % FIXME: não entendi esse ponto
...
\begin{itemize}
    ...
    \item Stop-the-line mindset % FIXME: não entendi esse ponto
    \item Revisão de código e padrões
    \item Active done-ness % FIXME
...
Este modelo foi criado por \citeonline{marick2003} para deixar clara a função de cada teste no projeto, gerando uma matriz 2x2 e resultando na seguinte divisão:
\begin{itemize}
    \item Eixo X
    \begin{itemize}
        \item \textbf{Testes que apoiam o time}: programadores utilizam estes testes como parte integral do desenvolvimento e para garantir que seu código funciona corretamente;
        \item \textbf{Testes que criticam o produto}: testes que não focam no desenvolvimento do produto, mas o produto final com o objetivo de encontrar falhas;
    \end{itemize}
    \item Eixo Y
    \begin{itemize}
        \item \textbf{Testes focados no negócio}: são testes que você descreveria para um especialista em negócio e que lhe despertaria interesse;
        \item \textbf{Testes focados na tecnologia}: testes que fazem parte do domínio da programação e desenvolvimento;
    \end{itemize}
\end{itemize}
% onde em um eixo estão os testes que apoiam a equipe (quadrantes 1 e 2) e os testes que criticam o produto (quadrantes 3 e 4), e no outro eixo são representados os testes focados no negócio (quadrantes 2 e 3) e os testes focados na tecnologia (quadrantes 1 e 4).

A numeração dos quadrantes não indica uma ordem a ser seguida, sendo apenas uma numeração arbitrária para facilitar a indicação de cada um (é mais fácil falar Q1 do que ``testes com foco em tecnologia que apoiam o time'').
...
%     \item Testes de aceitação do usuário
%     \item Alpha/beta
% \end{itemize}

% \subsubsection{Quadrante 4}
% \begin{itemize}
%     \item Testes de performance e carga
%     \item Teste de segurança
%     \item Testes de ``-ilidade''
% \end{itemize}
