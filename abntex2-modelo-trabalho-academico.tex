...
\begin{siglas}
    ...
    \item[FDD] \emph{Feature Driven Development} - Desenvolvimento Focado em Funcionalidades
...
Este pilar foca no aspecto técnico, das ferramentas e execução da pirâmide de teste, da integração contínuo e de outras práticas. Normalmente é o lado que mais atrai os desenvolvedores e empresas pois apresenta ferramentas e tecnologias para auxiliar na resolução dos problemas.

As pirâmides de testes apresentadas na \autoref{fig:testing-pyramid-combined} são baseadas no conceito criado por \citeonline{cohn2009}, e a ideia principal é indicar que devem existir mais testes de baixo nível (unitários) do que testes de alto nível (interface). \citeonline{fowler2012} acredita que se uma falha for detectada em um teste de alto nível então além de um \emph{bug} no código também foi detectada uma deficiência nos testes unitários.
...
\begin{itemize}
    ...
    \item Nível de serviço ou intermediário: FitNesse\footnote{Testes de aceitação: \url{http://www.fitnesse.org/}}, Robot Framework\footnote{Framework em Python para testes de aceitação: \url{http://robotframework.org/}}, JBehave\footnote{Framework em Java para Desenvolvimento focado em Comportamento (\emph{Behaviour-Driven Development} - BDD: \url{http://jbehave.org/}}, Cucumber\footnote{Outro sistema de BDD escrito em Ruby e compatível com diversas linguagens e frameworks: \url{https://cucumber.io/}};
...
\citeonline{galen2015} faz um aviso importante: não tentem automatizar 100\% dos testes. Isso é um objetivo fútil, e o que determinará o número ideal de testes é a análise das histórias do usuário para que exista uma cobertura adequada em todos os níveis, não podendo de levar em consideração o valor e complexidade na criação de cada um.
